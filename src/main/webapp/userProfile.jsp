    ...
    <section class="jumbotron text-center" style="display: flex; flex-direction: column; align-items: center;">
        <form  method="post" action="UserController?action=edit">
            ...
            <div class="form-group">
                ...
                <input type="text" name="username" class="form-control" id="username" disabled required value="<%=user.getUserName()%>">
            </div>
            <div class="form-group">
                <label for="password">Password</label>
                <input type="password" name="password" class="form-control" id="password" required disabled value="<%=user.getPassword()%>">
            </div>

            <div class="form-row">
                <div class="form-group col-md-6">
                    <label for="firstname">First Name</label>
                    <input type="text" name="firstname" class="form-control" id="firstname" required disabled value="<%=user.getUserFirstName()%>">
                </div>
                <div class="form-group col-md-6">
                    <label for="lastname">Last Name</label>
                    <input type="text" name="lastname" class="form-control" id="lastname" required disabled value="<%=user.getUserLastName()%>">
                </div>
            </div>
            <div class="form-group">
                <label for="dob">Date of Birth</label>
                <input type="text" onfocus="this.type='Date'" name="dob" class="form-control" id="dob" required disabled value="<%=user.getDob()%>" >
            </div>
            <div class="form-group">
                <label for="street">Address</label>
                <input type="text" name="street" class="form-control" id="street" disabled value="<%=user.getStreet()%>" >
            </div>
            <div class="form-row">
                <div class="form-group col-md-6">
                ...
                <div class="form-group col-md-4">
                    ...
                    <input type="text" name="state" class="form-control" id="state" disabled value="<%=user.getState()%>">
                </div>
                <div class="form-group col-md-2">
                    <label for="postalcode">Postal Code</label>
                    <input type="text" name="postalcode" class="form-control" id="postalcode" disabled value="<%=user.getPostalCode()%>" >
                </div>
            </div>
            <div class="form-group">
                <label for="phone">Phone Number</label>
                <input type="text" name="phone" class="form-control" pattern="[04]{2}[0-9]{8}" id="phone" title=" Phone number with 04 and remaining 8 digit with 0-9" required disabled value="<%=user.getPhoneNumber()%>">
            </div>
            <div class="form-group">
                <label for="paymentpreference">Payment Preference</label>
                <input type="text" name="phone" class="form-control" id="paymentpreference" disabled value="<%=user.getPaymentPreference()%>" >
            </div>
            <div>
                <button style="width: 40%; " type="button" onclick="HandleEdit()" class="btn btn-info">Edit</button>
            </div>
            <div style="text-align: right; margin-top: 40px">
            <button style="margin: 20px;" type="submit" disabled id="saveButton" class="btn btn-primary">Save Change</button>
                <%
                    if (user.getRole().equalsIgnoreCase("admin")){
                %>
